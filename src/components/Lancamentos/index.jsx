  ...
  return (
    <section className="lancamentos">
      ...
      <div className="alinhamento">
        ...
        <img src={cor} alt="Modelo usando os batons de demonstração" id="modelo" />

        <article className="box-testagem">
          <img src={estrelas} alt="Estrelas de avaliação" id="estrelas" />
          <h4>Matte Premium</h4>
          <p className="subtitulo">Cores disponíveis</p>

          <div className="carrossel-container">
            <button className="nav-btn" onClick={scrollLeft}>{" "} <img src={setaEsquerda} alt="ícone de seta para a esquerda" id="seta-esquerda" /> {" "}</button>

            <div className="carrossel" ref={carouselRef}>
              <button id="azul" onClick={() => setCor(azul)}></button>
              <button id="vermelho" onClick={() => setCor(vermelho)}></button>
              <button id="marrom" onClick={() => setCor(marrom)}></button>
              <button id="base" onClick={() => setCor(base)}></button>
              <button id="rosa" onClick={() => setCor(rosa)}></button>
              <button id="vermelhoR" onClick={() => setCor(vermelhoRosado)}></button>
              <button id="lilas" onClick={() => setCor(lilas)}></button>
              <button id="marromE" onClick={() => setCor(marromEscuro)}></button>
            </div>

            <button className="nav-btn" onClick={scrollRight}>{" "} <img src={setaDireita} alt="ícone de seta para a direita" id="seta-direita" /> {" "}</button>
          </div>
